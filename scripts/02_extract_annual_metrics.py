...
def select_annual_flow_facts(facts):
    ...
    df = df.sort_values(["fy", "filed_dt"])
    df = df.drop_duplicates(subset=["fy"], keep="last")
    df = df.drop(columns=["filed_dt"])
...
def select_annual_point_facts(facts):
    ...
    # For point-in-time metrics, use the latest end date and latest filed value for each fiscal year.
    df = df.sort_values(["fy", "end_dt", "filed_dt"], ascending=[True, False, False])
    df = df.drop_duplicates(subset=["fy"], keep="first")
    df = df.drop(columns=["filed_dt", "end_dt"])
...
def extract_metric(companyfacts, metric_name, metric_type):
    ...
    for tag_priority, tag in enumerate(candidate_tags):
        facts = get_facts_for_tag(companyfacts, tag, unit)
        ...
        annual_df = annual_df[
            ["fy", "start", "end", "period_days", "val", "form", "filed", "accn", "fact_type"]
        ].copy()
    ...
    combined_df["filed_dt"] = pd.to_datetime(combined_df["filed"], errors="coerce")

    combined_df = combined_df.sort_values(
        ["fy", "tag_priority", "filed_dt"],
        ascending=[True, True, False],
    )

    selected_df = combined_df.drop_duplicates(subset=["fy"], keep="first")
    selected_df = selected_df.drop(columns=["tag_priority", "filed_dt"])
    selected_df = selected_df.sort_values("fy").reset_index(drop=True)

    return selected_df


def build_annual_table(companyfacts, analysis_years=ANALYSIS_YEARS):
    metric_frames = []

    for metric_name in FLOW_METRIC_TAGS:
        metric_frames.append(extract_metric(companyfacts, metric_name, "flow"))

    for metric_name in POINT_METRIC_TAGS:
        metric_frames.append(extract_metric(companyfacts, metric_name, "point"))

    raw_df = pd.concat(metric_frames, ignore_index=True)

    if raw_df.empty:
        raise ValueError("No annual facts found. Please check the SEC JSON file.")

    available_years = sorted(raw_df["fy"].dropna().unique())
    ...
    if len(available_years) < analysis_years:
        print(
            f"  [WARNING] Only {len(available_years)} fiscal years of data available "
    ...
    required_years = available_years[-(analysis_years + 1):] if len(available_years) > analysis_years else available_years
    analysis_year_list = required_years[-analysis_years:] if len(required_years) > analysis_years else required_years

    raw_df = raw_df[raw_df["fy"].isin(required_years)].copy()
    ...
    pivot_df = pivot_df.sort_values("fy").reset_index(drop=True)

    return raw_df, pivot_df, analysis_year_list
...
def calculate_metrics(pivot_df, analysis_year_list):
    df = pivot_df.copy()

    required_columns = list(FLOW_METRIC_TAGS.keys()) + list(POINT_METRIC_TAGS.keys())
    ...
    df.loc[df["total_debt"] == 0, "total_debt"] = df["long_term_debt_total"]

    df["invested_capital"] = df["total_debt"].fillna(0) + df["equity"].fillna(0) - df["cash"].fillna(0)

    df["fcf"] = df["operating_cash_flow"] - df["capex"].abs()
    df["fcf_margin"] = safe_divide(df["fcf"], df["revenue"])

    df["book_value_per_share"] = safe_divide(df["equity"], df["shares_diluted"])
    df["dividend_per_share"] = safe_divide(df["dividends_paid"].abs(), df["shares_diluted"])

    df["net_margin"] = safe_divide(df["net_income"], df["revenue"])

    df["average_equity"] = (df["equity"] + df["equity"].shift(1)) / 2
    df["roe"] = safe_divide(df["net_income"], df["average_equity"])

    df["debt_to_equity"] = safe_divide(df["total_debt"], df["equity"])

    df["revenue_growth_yoy"] = df["revenue"].pct_change()
    df["eps_growth_yoy"] = df["eps_diluted"].pct_change()
    df["fcf_growth_yoy"] = df["fcf"].pct_change()

    analysis_df = df[df["fy"].isin(analysis_year_list)].copy()
    ...
    return per_share_df, cash_flow_df, profitability_df, balance_sheet_df
...
def save_to_excel(
    ...
    output_path = EXCEL_DIR / f"{ticker}_annual_fundamentals.xlsx"

    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        per_share_df.to_excel(writer, sheet_name="per_share", index=False)
        ...
        raw_df.to_excel(writer, sheet_name="raw_annual_facts", index=False)
        notes_df.to_excel(writer, sheet_name="data_notes", index=False)
...
def main():
    ...
    companyfacts = load_companyfacts(ticker)

    raw_df, pivot_df, analysis_year_list = build_annual_table(companyfacts)
    ...
    (
        ...
        profitability_df,
        balance_sheet_df,
    ) = calculate_metrics(pivot_df, analysis_year_list)

    notes_df = build_data_notes(raw_df)

    print("Analysis years extracted:")
    print(", ".join(str(year) for year in sorted(analysis_year_list)))

    print("\nRaw years kept for YoY calculation:")
    print(", ".join(str(year) for year in sorted(pivot_df["fy"].tolist())))

    print("\nKey metrics preview:")
    preview = profitability_df[["fy", "revenue", "net_income", "net_margin", "roe"]]
